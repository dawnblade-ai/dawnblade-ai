...
const KEYWORDS = {
   ...
   "ice fusion":          {status:"partial",     note:"v4.21 — THE RECORD WAS STALE (v3.69: when a record says a thing is unbuilt, ask the engine). `fx.fusionCost` parses the keyword line into its talent list, `execute` settles `fused` at the top of the play, and the riders that ask 'if this was fused' are answered as a cond — all four Aether Icevein records and Brain Freeze read `full`. PARTIAL rather than live because the printed cost is 'you MAY reveal', and the engine auto-takes it whenever the hand holds a qualifying card; the SIY014 face carries the reminder text ('As an additional cost to play this, you may reveal an Ice card from your hand') and revealing spends nothing, so auto-taking is nearly always right — but it is an approximation and `execute` says so in its own comment"},
   ...
   "lightning fusion":    {status:"partial",     note:"v4.21 — the same mechanic one talent over, and the same stale record: `fx.fusionCost` reads it, Arcanic Shockwave and Entwine Lightning both `full`. PARTIAL for the same reason as ice fusion — the printed 'you may reveal' is auto-taken"},
