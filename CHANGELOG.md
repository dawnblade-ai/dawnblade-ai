...
> by fewer than 2 cards, it gets +3{p}."* — WIDOWMAKER ×3, Azalea's
>
> *"**Defense reaction cards can't be played this chain link.** When this hits a
> hero, you may turn a card in their arsenal face-up, then destroy a defense
> reaction in their arsenal."* — WRECK HAVOC ×3, Dorinthea's
...
whole reaction step and gave `sparring.act` a caller for it** — the ladder plays
**824 defence reactions per 630 games** — so the sentence was false at the TABLE
from that version on, while staying true of the trainer, whose opponent fabricates
its swing. v3.16's shape: a noop describing a SIBLING board. Measured, the
restriction had **no reader on either board**: grep finds nothing for the field,
the phrase or any name like it.

**THREE BLINDNESSES, NOT TWO.** Coverage counts a `noop` as ACCOUNTED FOR — this
file's own *"first place to look"*. The one-sided fairness sweep looks for a card
**STRONGER** than printed, and a restriction on the OPPONENT dropped is the other
...
Worth recording as its own gap: that census is what a reader would expect to
catch this, and it is looking at the wrong property.

**IT IS A CARD FACT, NOT AN OP, AND AN OP WOULD HAVE BEEN WRONG RATHER THAN MERELY
DIFFERENT.** An attack card's `fx.ops` ride to **RESOLUTION** (v4.08) — hundreds
of lines after the defend and reaction steps this restriction governs — so a
restriction pushed as an op arrives once the window it closes has already shut. It
rides as `fx.noDrx` and is answered by `parser.drxBarWhy`, with `fx.gyFirstGa` as
the shape verbatim and v4.01's rule coming with it: **a clause-level field only
exists if the clause loop FORWARDS it.**

**"THIS CHAIN LINK" IS `pend`'s OWN LIFETIME**, which is why nothing is banked and
nothing expires: judge replaces `pend` at every declaration and clears it at the
close, so the bar is DERIVED off the open link (`wardValue`'s rule, v4.34;
`runeCount`'s, v2.23; `chiFloating`'s, v4.54). A side field would have needed the
per-link sweep `blockH`/`blockG` get in `strike` (v2.46) **and** a `WIRE_V` bump,
for a fact the link already carries.

**ONE BODY, THREE DOORS** — `judge.legal` and the trainer's two (`playRx` from
hand, `playRxA` from arsenal). It returns the **SENTENCE** rather than a boolean
for v4.59's `promptZoneWord` reason: two spellings of one refusal across two
boards is the mirror the no-mirror rule is for, and in a training sim the feed is
the lesson (v3.60, v4.24) — the line **names the card that closed the window**, so
a player learns why rather than being told no.

**REFUSED BEFORE THE CARD LEAVES THE HAND**, beside `nextTurnBars` and v3.11's
printed target restriction: a defence reaction refused after it has resolved is a
player losing a card to a play the rules never allowed. **The policy needed no
change at all** — `sparring.act` filters every proposal through `judge.legal`, so
it inherits the rule and proposes `pass` instead, which is its own contract doing
its job.

**THE TRAINER'S HALF IS LATENT, AND THAT IS MEASURED RATHER THAN ASSUMED.** Both
doors are only reachable in `mode:"block"`, which `foeVanilla` enters with the
swing fabricated as the `[3,4,5]` escalation — no card, so nothing can print the
restriction (the same measurement from the other end is that both doors already
hand `defendValue` a null attacking card). The site exists anyway (v3.01) and is
drilled with a synthetic `pend` (v3.73), or a sabotage that deletes either door is
silent.

### THE THIRD WORDING IS NARROWER, AND IT IS DELIBERATELY REFUSED

> *"Your next arrow attack this turn gets +3{p} and \"**Defense reactions can't be
> played FROM ARSENAL this chain link.**\""* — RELEASE THE TENSION ×3, Azalea's

**THE NOTE THIS VERSION WAS PLANNED FROM SAID TWO CARDS.** Measured, the family is
**THREE cards and nine records** — v4.09 again, and the note was two days old and
mine. The third is a **quoted granted ability** and a genuinely different rule: it
restricts one **ZONE**, so read by the unconditional anchor it would bar a defence
reaction from the **HAND** as well — stronger than printed, the direction that
steals games, and v2.30's arrow buff on a sword one restriction over.

So the anchor stops short of it, the card keeps the honest `fx.quotedUnread` flag
v3.41 built for exactly this, and **the zone is not carried as a parameter** —
with only the two unconditional records reading, a `zone` field would have no
claimant, which is dead rules code that reads like a rule (v4.11, v4.52). What it
waits on is `quotedRider`, which requires `sub.onHit` and is an ON-HIT reader by
construction, so carrying a static restriction there means widening the rider
shape for all **four** grant families that share it. `drx-bar-from-arsenal-unread`,
`stated`, with a driven probe — **and the near-miss is a REAL pool card**, which is
rarer and better than a synthetic (v4.18), in the hero who decks both halves.

**"CARDS" AND "REACTIONS" MEAN THE SAME THING HERE, MEASURED.** In the CR the bare
plural also covers a defence-reaction ABILITY; over 797 records the pool prints
**ZERO** of them (`classifyClause` guards `Action -`, `Instant -` and
`Attack Reaction -` and there is no fourth prefix) and `abWindow` has no such
window to answer. So `isDR` — the printed TYPE — is the whole of what either
wording can reach, and an **INSTANT** played in the defence window is not a
defence reaction and is correctly untouched.

### ELEVEN COMMENTS NAMED A FUNCTION THAT LEFT AT v2.71

**FOUND BY WRITING ONE.** This version needed to say which trainer function enters
...
**AND `test/actor.test.js` HAS RECORDED BOTH SINCE — including naming `foeVanilla`
as the escalation's home in as many words.** So the ledger knew, and eleven shipped
comments across five files went on naming the dead ones in the **present tense**
for eighty-nine versions: `judge.js`'s own two-combat-paths diagram,
`sparring.js`'s opening paragraph, two sites in `effects.js`, one in
`tools/approx.js` and five in `index.html`. v2.53's rule one step out — **an ANCHOR
naming a deleted function keeps reporting green, and PROSE naming one sends the
next reader hunting a function that is not there** (v4.09: this project's own
comments are the most convincing source of a name that does not exist).
...
`RETIRED_NAMES` is pinned, and membership is the judgement: **a name goes in it
when it has a live SUCCESSOR to redirect to.** `allySwing` is deliberately NOT in
it and is the worked example — retired at v3.44, every remaining site past tense by
construction, and no single successor function to name instead. Its sites are
pinned by count with a **per-site** tense cue, because a file-wide check passes on
the first past-tense sentence whatever the others say (v3.26) — **and the count was
THREE where I had written two from memory**, which is v4.09 for the third time in
one version.

### MEASURED

- **Exactly 6 records' parse moves and 0 tiers do.** The clause goes `noop` → `run`
  and gains the field; 751 full / 35 part / 11 none is **unchanged**, the audit is
  397 full / 8 part / 0 none unchanged, and the audit diff is one timestamp line
  plus one clause marker — so no floor repin (v4.18).
- **Release the Tension's parse is byte-identical**, which is what says the anchor
  did not widen onto it.
- **The ladder moves ONE game in 630 and it is inside the band**: arakni 79 → 78,
  dorinthea 75 → 76, every other hero identical, against a noise band of median 3
  (v4.40). Stalls unchanged at the one Enigma–Gravy draw; **POLICY REFUSALS 0,
  INVARIANT VIOLATIONS 0.**
- **The ROUTE is driven, and the evidence is the counter it removes:** `reaction`
  **824 → 816** over 630 games — eight defence reactions denied — with
  `layer 929 → 922` and `crush 915 → 910` following them. **No route counter is
  added**, because the event is a REFUSAL: `judge.legal` returns a reason and logs
  nothing, so a counter could only ever read 0 and a number that cannot see its own
  event is v4.46's defect (v4.24, v4.41, v4.52, v4.57).
- `npm test` **2993 / 0 fail / 5 skipped**; scenes 99/0; fairness CLEAN and UNFAIR
  0; `crindex --check` green; the anchor sets unchanged and the new reading is
  REACHED; both babel blocks compile.
...
> — CRASH AND BASH ×3, a Block card
>
> *"When this defends, you may pay {r}. If you do, it gets +2{d}"*
> — BROTHERS IN ARMS ×3, Kayo's and Gravy Bones' lists

**THE TRAINER RAISES TWO WALLS AND ONLY ONE OF THEM REACHED A SHARED
`defends` BODY.** When the player ATTACKS, `resolvePlay` calls
`afterDefenders` and gets all four *"when this defends"* families — clash,
`optCost`, `millCost`, `payCost`. When the player BLOCKS, which in the
trainer is most of the game, `takeIt` called `resolveClash` and its own
hand-rolled `payCost` scan and **nothing else** — so Crash and Bash's
`optCost` (3 records) and Washed Up Wave's `millCost` (1) were never offered
on the wall the player raises.

v3.01's shape with the boards swapped, recorded at v4.53 as
`trainer-blocks-wall-no-defends-body`, `open`, with a driven probe. **The
probe went RED the moment the body landed**, which is what an `open` record
is for (v4.02).

**IT COULD NOT SIMPLY CALL `afterDefenders`, AND THAT IS WHY THE FIX IS AN
EXTRACTION — A DISTINCTION THE RECORD DID NOT DRAW.** That function opens
`if(!card) return n;` and on the block path there IS no attacking card,
because `foeSwing` fabricates the swing as the `[3,4,5]` escalation. So the
call would have returned immediately, done nothing, and **looked exactly like
a fix** (v3.50, sixth outing). What needs the card is PHANTASM alone; the
four families are about the DEFENDERS. `effects.defendsTriggers` is the one
body now and `afterDefenders` is one of its two callers.
...
`1 - actorOf`; on the block path the player defends AND acts, so it is
`actorOf`. A body that derived it would be right on one wall and wrong on the
other — `tapFoeHero`'s inversion (v3.48).

**AND THE PAUSE GENERALISED RATHER THAN GROWING.** `finishBlock` totals the
wall and strikes damage on the next line, and both `millCost` and `payCost`
pay out a `defBuff` — so a queued sheet has to be answered BEFORE the total.
`takeIt` holds the wall with `_wallPending` and `openPrompt` — the one place
that knows the queue is empty — resumes `finishBlock`, **deleting the marker
before it calls**. That replaces `defpay`'s entire private copy of the same
control flow: a mode, a queue, an action-bar branch, a statusline and a peek
chip, all for one family. 922 bytes of `index.html` left.

**AND CLOSING IT CLOSED A SECOND DEFECT THE RECORD NEVER NAMED.**
`confirmDefPay` paid out of FLOATING resources only — and **CR 4.4.3e takes
every floating resource from BOTH seats at every end phase**, so on the
opponent's turn, which is the only turn this wall is raised on, the player
held **0** and the Pay button was disabled. Brothers in Arms' printed +2{d},
built at v4.53 *for this exact wall*, was unreachable unless the player
happened to have overpaid for an instant in the same window. The shared sheet
pitches on demand (RULING 2026-08-01), which is what the DUMMY's wall has
always done with the identical card.

**NO TOOL HERE COULD SEE ANY OF IT.** All four records read `tier: full`,
because the clauses ARE consumed; a defender worth LESS than printed and a
printed line of play never offered are both the direction the one-sided
fairness sweep is built not to look in; and `npm run play` drives
`judge.reduce`, so **no ladder number can speak about a trainer wall at
all** — which is why the route is DRIVEN and no route counter is added
(v4.24, v4.41, v4.52).

**`applyDefMod` LEFT THE CLOSURE'S EXPORTS, AND THAT IS A DEPARTURE RATHER
THAN A DROP.** Its one outside caller was `confirmDefPay`; the shared sheet's
payout goes through `applyAnswer`, inside the module. `defBuffOf`'s trainer
callers go **4 → 2** for the same reason — the `defpay` cycle and that
sheet's own label are gone, so two hand-rolled readers left rather than two
askers. A name leaving a census is as deliberate an edit as one arriving
(v4.12), and both counts are pinned.

### Measured

- **No pool record's parse or tier moves** — 395 full / 10 part / 0 none,
  unchanged, because no reader changed (v4.18's point). The `AUDIT.md` diff is
  one timestamp line, so no floor repin.
- **`npm test` — 2911 drills, 0 fail, 5 skipped.** `test/defwall.test.js` is
  the new file (16 drills): the four families pinned as SETS both directions,
  `defendsTriggers` driven at both seats and both walls, the body proved NOT
  to drain, and four SOURCE scans with their reach STATED.
- **`npm run scenes` — 98 passing, 0 failing.** `npm run fairness` CLEAN;
  `npm run sweep` **0 entries break a rule**; `crindex --check` clean;
  `npm run anchors` unchanged.
- **The approximation ledger: `open` 7 → 6, `closed` 15 → 16.** Two records
  moved — `paycost-defends-at-the-table` had its tail flipped to assert the
  trainer's own scan is gone, and `trainer-blocks-wall-no-defends-body` is
  closed with its probe turned round.
- **The ladder is BYTE-IDENTICAL at three seeds on both sides**, run rather
  than reasoned about (v4.43). That is the right result and it says something
  narrow: the ladder can only see the EXTRACTION, not the new wall, so what it
  proves is that `afterDefenders` behaves identically through the shared body.

### AND A DRILL THAT WAS READING THE DOCUMENTATION, FOUND BY THE APP_VER COMMENT
...
`CR 4.4.3e`** — and that comment is 3,400 lines above the end phase, so the
slice's END moved above its START and the drill went red. v4.27's defect in the
one line that ships to every player, and the project's own answer applies:
**reword the prose, do not weaken the scan.**

**BUT THE DRILL HAD BEEN READING PROSE ALL ALONG, AND PASSING BY COINCIDENCE.**
Three comments cite `CR 4.4.3d`, the first 394 lines above the step marker — so
`indexOf` had never once found the step. **The slice was 24,952 characters
where the step is 1,680**, fifteen times the code it meant to read, and the one
`weaponUsed = {}` it found was the right one by accident.

**THE FALSELY-GREEN DIRECTION IS THE DANGEROUS ONE AND IT IS DEMONSTRATED.**
Delete the untap from step (d), plant `weaponUsed = {}` in a comment 370 lines
up, and the old anchor answers **TRUE** — the drill passes with a CR rule's site
gone. The new one bites. Its two siblings in the same file already spelled the
`" —"` the step markers carry; this one did not.

**AND MY FIRST DECOY COULD NOT EXPRESS THE BUG** (v3.62): planted before the
first citation, it fell outside the slice and both anchors read `false`, which
reads exactly like a tightening that buys nothing. The decoy has to sit AFTER
the anchor it is fooling.

### AND THE SAME FAMILY, CENSUSED — `test/slicecensus.test.js`

v4.21's rule: when you fix two members of a family, census the family. Both of
this version's drill findings are **a source slice whose anchors did not bound
the body the drill names**, so the census measures every one of them.
...
check that flagged every comment anchor would sit red on every ship, which is
how `UNFAIR` came to read 1 for nineteen versions. **18 slices, and the widths
and the FILE DISTRIBUTION are pinned as data**: one growing, or one changing
which file it reads, is a deliberate edit.
...
blind to the SAFER form this version's two corrections now use (bounding at the
next same-indent declaration with a `search`), and blind to a fixed-width
window, which fails in the OPPOSITE direction (v4.05: too narrow INVENTS
findings). Both blind spots are pinned by the pattern's own control.

**AND THE INSTRUMENT HAD EVERY FAULT IT WAS BUILT TO FIND.** Four, in order:

- **it reported a slice of `judge.js` that no drill takes** — the CR 4.4.3e/f
  markers exist in both files, so pairing each literal against every file where
  both anchors happen to exist invented a row. v4.00's false POSITIVE, in the
  instrument for it. The holder's own assignment decides now.
- **it found ITSELF**, because its header spelled the pattern out with quoted
  anchors. Third time in this version.
- **stripping comments to fix that ate a legitimate anchor** —
  `phasebar.test.js` bounds a slice on a literal that IS a block-comment
  divider, which is what `html-balance.test.js`'s pre-neutralize list exists
  for. So the prose was reworded instead, per this project's own rule.
- **and writing that divider into the note closed the note early**, so the file
  stopped parsing: the orphaned-terminator bug that shipped in v2.27 and broke
  the whole page, inside a comment about stripping comments.

**THREE MORE FAULTS WERE MINE AND EACH IS A NAMED SHAPE.** Its own control
asserted the pattern REJECTS a fixed-width window; measured, the pattern matches
it and the `ib <= ia` guard is what drops it — so what is pinned is which guard
does the excluding. The holder resolver had no drill at all until a sabotage
forced every holder to one file and came back **SILENT**, because the single
non-`index.html` row simply dropped out and the count stayed above the
alive-check's floor: **a census that loses a row reads exactly like one with
nothing to report** (v3.81, v4.07). And a `resolveClash` anchor was written from
the signature I remembered rather than the one in the file (v4.09).

### Five pins moved, each read before it was touched

A pin that is edited without being read is a guard being switched off. Every
one of these is a claim that got STRONGER:

- `test/bravo.test.js` and `test/defmod.test.js` anchored the family scan at
  `afterDefenders`, which was only ever **half** the claim — v4.53 measured
  that a second wall existed and that function did not serve it. Both anchor
  at `defendsTriggers` now and assert **both** callers, with the slice bounded
  at the next declaration (v4.05).
- `test/clash.test.js` asserted `takeIt` calls `_EFX.resolveClash` directly.
  Widened rather than relaxed: the block path must call the shared body, must
  **not** reach past it to the clash half alone, and the shared body must run
  the clash on the caller's seat **before** the sheets — a clash moves a
  defender's value, so a prompt answered first resolves against a board the
  clash has not yet changed.
- `test/actor.test.js`'s ledger gained `defendsTriggers` as a MIGRATED body
  and lost `confirmDefPay`'s anchor. **An anchor that names a deleted function
  keeps reporting green** (v2.53), so it is deleted with a recorded reason
  rather than left pointing at nothing.
- `test/defmod.test.js`'s `defBuffOf` caller count, above.
...
**THE SECOND HALF OF A PRINTED COST WAS SILENTLY DROPPED, AND THE PROOF IS
THAT IT PARSED BYTE-IDENTICALLY TO A CONTROL.** Handed only
`Instant - Destroy this: Prevent the next 1 arcane damage…`, `parseHeroPower`
answers the same `cost`, the same `sd`, the same `label` — so the ability was
offered, resolved, shattered the piece, and left on the board the Runechant
the card is printed to spend. **In the one hero whose whole engine IS
Runechants**: each one kept is a point of arcane damage on his next swing
that the card had already charged him for. Live, in Viserai's chest slot.
...
and coverage counts the text accounted for. And the one-sided fairness sweep
models a PAYLOAD read too generously, never a **COST that was skipped**
(v4.27's fusion, one verb over; v4.33's charge, one more).

### `test/costcensus.test.js` — EVERY ATOM OF AN ACTIVATION COST

`tools/ledger.js` censuses KEYWORDS, `tools/approx.js` the RULES MACHINE,
`test/fxcensus.test.js` the parse's own fields, `test/condcensus.test.js` the
conditions. **Nothing asked it of a COST** — and a cost is the half of an
activated ability where being wrong is free value rather than a missing
feature (v2.04).

Measured: the pool prints **29 distinct activation costs over 104 records**.
Each is split into ATOMS and each atom names the flag or the named reader
that accounts for it — and **the Robe was the one ACCEPTED cost carrying an
atom no flag named.**

**IT IS DRIVEN, NOT A TABLE OF REGEXES AGAINST REGEXES** (v3.56). The
assertion is that the engine's real parse carries the flag, so the census is
green against a pattern and red against an engine. **Verified by pointing it
at the pre-fix parser**: it goes RED naming the card, the cost, the atom and
the missing flag. Neutering the driven half instead costs exactly one
failure of the eight, which is what says the DRIVE is carrying it.

**BOTH DIRECTIONS ARE PINNED** (v4.12, v4.17): the nine readers the pool
reaches AND the four printed atoms this reader refuses. A reader with no
claimant is dead rules code that reads like a rule (v4.11, v4.50, v4.52) —
and the drill's own first draft carried two such rows, for a counted soul
banish and the Gun's steam counter, both already described by a general
sibling. The pin failing is what found them.

### THE WHOLE BUILD WAS THE PARSER

`equipPiece` has stamped `_destroyBoard` **since v3.86**, as a latent guard
whose own comment says *"no pool EQUIPMENT prints this cost today"* — the
measurement this version moves. `judge.abCostWhy` and the trainer's
`tryPlay` both refuse a cost naming a permanent the seat does not control,
and `execute` destroys it into the **turn-stamped** graveyard through
`payLeave`. **Before building machinery for a shape, check whether the
machinery is the shape you already have** (v3.58, v3.73).

**IT IS AN OPTIONAL MIDDLE, NOT A SECOND READER** — v3.79's Radiant Touch
(*"Banish **this and** a card from your soul"*) one cost verb over, and
v4.14's Carrion Crown comma-joined.

**AND `/i` IS NOT THE FIX.** The same printed verb arrives in two cases:
Gravy Bones' follows a pip (*"{t}, destroy a Gold…"*) and is lowercase; the
Robe's OPENS the cost, so the printing capitalises it. An `/i` flag reads
...
called "card". The case-sensitivity three tokens later is v3.53's guard and
is what the flag would delete, so the one letter that legitimately varies is
spelled `[Dd]` instead. **Found by driving it**: the first draft of the
widening came back byte-identical to the old answer, because the Robe never
reached the branch at all.

**READ WHOLE OR REFUSE** (v2.29). `sd` is a LOOSE test for the word
"destroy" anywhere in the cost, which is exactly what let the Robe fall
through to the generic reader and pay half its price — so a
`destroy this and <something this reader cannot name>` now refuses the whole
line rather than reading as a bare `destroy this` at a discount. Latent and
measured; drilled with a synthetic (v3.73) **and with its positive control**,
because a guard that refuses everything passes the four negative rows
perfectly (v3.98).
...
the HERO builder passes false and a hero destroying "this" destroys the hero;
this branch never had to ask while it always answered `sd: false`, and the
moment it can answer TRUE it inherits the question. **The first draft without
it handed a hero record a self-destroying cost where the old reader had
refused outright** — caught by measuring the blast radius at both
`allowDestroy` values rather than one.

### Measured

- **exactly 1 pool record's parse moves, 0 tiers move** (394 / 11 / 0
  unchanged — v4.18's point: the card was already `full`), so the coverage
  floor needs no repin and the audit diff is one timestamp line.
- **Gravy Bones' cost is byte-identical** and pinned as the control.
- **the ladder is BYTE-IDENTICAL at three seeds on both sides**, run rather
  than reasoned about (v4.43) — and that is about the LOADOUT: `defaultPicks`
  ranks a chest by printed defence and takes Beckoning Haunt (2) over the
  Robe (0), so a driven game never wears it. v4.43's Hood, v4.49's Gun,
  v4.52's Silent Stilettos — **fourth outing**. No route counter is added
  for that reason: a number that can only read 0 because of the loadout is a
  number about the loadout (v4.24, v4.29, v4.41, v4.52). The premise is a
  drill, and the accept path is driven end to end plus a scene.
- **`npm run scenes viserai` 2/2 → 3/3**, and the new scene FAILS against
  the pre-fix engine on the game fact itself — *"none survives to pop on the
  next swing: 1 (want 0)"*.
- fairness CLEAN, sweep UNFAIR **0**, `crindex --check` 0.
- **16 sabotages, 16 bite.** One came back silent on the first pass and it
  was my own drill: the trainer scan pinned the bare identifier, so
  `if(… && false)` passed it — v4.00's defect verbatim and v4.54's lesson one
  cost over. **The whole conditional is pinned now**, opening paren included,
  comments stripped with the stripper's control routed through the scan
  (v4.27, v4.32). A second needed a two-part sabotage, because a census with
  no faults cannot express a weakened check on its own (v3.62) — neutering
  the drive against the PRE-FIX parser is what shows the drive carrying it.
- and a drill named `INV` without requiring it, which only DRIVING found
  (v4.09, again).
...
diagnostic in this project says which half was the blocker (v3.79, v4.43):
hand the same printed line a trigger that already has a reader and it
parses **in full** — cost 3, the "you may", the payload, all of it. So the
TRIGGER was the whole gap and every piece of machinery was already here.
...
answers `[["ap",1]]` — the payload with the printed **DESTROY** silently
dropped, which is v4.37's Mark of the Huntsman and v4.25's Boom Grenade one
joiner over. A trigger built without the split hands its controller an
**unbounded, repeatable free action point** off a permanent that never
leaves. **v3.72's rule: when you build a SOURCE, ask which payload paths it
has just made reachable.**

### ONE PRINTED CLAUSE NAMES TWO EVENTS

`entersLeaves`'s shape (v3.20) — one trigger name, two sites answer to it.
Splitting it into two triggers would make `payCost.trigger` a list and give
every existing record a one-element array for a card that names one event.

**THE TWO HALVES ARE NOT EQUALLY REACHABLE, AND THAT IS MEASURED.** Every
pool record a phantasm pop can destroy is an `Illusionist Action - Attack`
(4 cards, 12 records) and Enigma decks four of them, so that half is
**LIVE**. The other half needs an ATTACKING ally to die and
`effects.allyDeath`'s one caller is handed `link.target` — the ally the
attack was aimed **AT** — so it is **latent**, gated on the caller's own
explicit answer (v3.69) and drilled with a synthetic (v3.73). Measured:
Silent Stilettos is the pool's **only** record printing "attacking ally".

**THE PRINTED TYPE RESTRICTION IS TESTED, NOT ASSUMED.** `isAtkActionCard`
changes no answer today — v4.31 is the version that found this very block
asking for a card's power and dropping both of the keyword's own printed
restrictions, so a fire site reasoning *"it can only be an attack action
card anyway"* is exactly the shape that cost.

### `payTrigger` IS ONE READER NOW, AND TWO OF ITS PHRASES HAD NO CARD
...
a card reaching either would parse a payload with no schedule — the single
shape `failstates.js` cannot see (v3.07). And the bare `this hits` is a
phrase **v3.45 measured and EXCLUDED by name**: "a hero" is part of the
trigger, so a bare form fires on a hit at an ALLY. Deleted, and the near-miss
is drilled from the reader itself rather than only through the gate.

**AND `OFFER_TRIGGERS` IS THE CAPABILITY SPLIT, PINNED AS A SET.** The two
cost verbs are not equally supported at every site: `offerPayCost` builds
its sheet through `payCostSpec`, which knows both prices, and `defends` is
the one trigger it never fires. So the DESTROY verb requires membership and
the PAY verb does not — stated as data, with a drill pinning the set against
`effects.js`'s own call sites in both directions.

### A THIRD FINDING: `defends` IS A ONE-BOARD RULE
...
for its printed 3 with a printed line of play that does not exist.

**v3.01's shape, found sideways by censusing the family.** Coverage cannot
see it (the clause IS consumed, `tier: full`) and the one-sided fairness
sweep cannot either (a defender worth LESS than printed is the direction it
does not look in). Recorded rather than half-built — it is not
`offerPayCost`'s shape as it stands, because a declared defender is in
neither the gear nor the arena, and the answer has to reach `defendValue`'s
`defMod` rather than the trainer's own `defBonus`. `tools/approx.js`:
...
### THE FAMILY'S OTHER GUARD WAS MISSING TOO

The `you may pay` branch tested only `ops.length`, which is the exact hole
v3.93's header describes and guards against in the destroy branch twenty
lines up: `classifyClause` answers `{status:"noop", ops:[["noop", …]]}` for
a keyword it deliberately does nothing about, so the length is 1 and the
cost is charged for a reward nothing delivers. **It matters more now**,
because a `payCost` can carry a printed destroy as of this version.

### THE FEED NAMED ONE HALF OF THE PRICE

`payVerb`'s destroy branch answered **before `prompt.cost` was ever read** —
right while the only records were v3.93's, where the destroy IS the whole
price, and wrong the moment a card charges both. It reads *"paid 3 and
destroyed Silent Stilettos"* now, and **v3.93's own two lines are
byte-identical**: `tools/selfplay.js`'s `destroycost` counter matches those
exact phrases, so a reworded line would report ZERO exactly as a missing
route does (v3.81). All four spellings are pinned against the counter's own
regex.

### MEASURED

- **Exactly ONE pool record moves, `part` → `full`** — 393 → 394 full,
  12 → 11 part (per unique card; 747 → 748 / 38 → 37 per record). Every
  sibling `payCost` is unchanged, verified by a canonicalised whole-pool
  diff in both directions. Floor re-pinned after reading the diff.
- **The ladder is BYTE-IDENTICAL at three seeds on both sides**, run
  rather than reasoned about (v4.43).
- **The route reads ZERO on the ladder, and that is about the LOADOUT.**
  Enigma lists two Legs pieces and `defaultPicks` ranks by printed
  defence, so it takes Blade Beckoner Boots (1) over the Stilettos (0) and
  the piece is never worn in a driven game — v4.43's Hope Merchant's Hood
  and v4.49's Plasma Barrel Shot, a third time. The premise is a **drill**,
  and the accept path is driven end to end by `test/stilettos.test.js` and
  an Enigma scene so the payload is not left with no caller (v3.50).
- **21 sabotages, 21 bite.** Three of the first pass came back SILENT and
  every one was my own fixture (v3.62): a near-miss that dropped the word
  under test **and** the whole second half of the phrase, so it was refused
  on the half that was not being tested; a `defends` fixture that reached
  only ONE of the two destroy branches, which the other covers (v4.50,
  verbatim); and a `this hits` sabotage masked by the `OFFER_TRIGGERS` gate,
  which is a refusal one layer below the one the drill was asking about.

## v4.51 — a tie that counts as a lead, one way only
...
The clause has read **nothing since the card was dealt**, and it is wrong in
**both directions at once** — which is exactly why neither the coverage audit
nor the one-sided fairness sweep could see it:

| | |
|---|---|
| **weaker than printed** | Lyath lost Mocking Blow's boo on a tie — and with it the Might token his own hero passive makes of one |
| **stronger than printed** | the opponent lost every *"if you have less {h}"* clause they hold — Fyendal's Fighting Spirit, Scar for a Scar, Wounded Bull, across five other decks |

**THE ASYMMETRY IS THE CARD.** Its controller counts as AHEAD; their opponent
counts as BEHIND. Nothing makes the controller count as behind and nothing
makes the opponent count as ahead, so `effects.lifeAhead` and `lifeBehind`
take their two sides in **opposite orders** on purpose:
...
Read the other way round, the holder turns on their own Fyendal's Fighting
Spirit and the opponent collects Mocking Blow's boo — two grants the printed
line withholds. All four combinations are drilled.

**EVERY CROSS-SEAT LIFE COMPARISON GOES THROUGH THAT ONE PAIR.** Measured,
`effects.js` holds exactly three sites and all three did it inline with
`act(n).hp > foe(n).hp`: `lifeGt` (Mocking Blow), `lifeLt` (four cards) and
`lifeLock`'s gate (Reaping Blade). A fourth written inline would silently
ignore the piece, so a drill scans for one — **with comments stripped and the
stripper's own control routed through the scan** (v4.32), because this
project's prose names the shape it forbids and a raw scan reports that
sentence as the defect (v4.27, `sync.test.js`).

**THE GRANT IS DERIVED FROM THE PERMANENT, NEVER BANKED** — `wardValue`'s
shape (v4.34) and `runeCount`'s before it. A gear piece is dealt straight into
the gear zone and never resolves, so there is no play moment to bank a side
field at, and a banked flag would outlive the piece anyway. Board **and** gear,
a destroyed piece grants nothing (v3.54), and the board half is latent and
drilled with a synthetic.

### A CONDITION NAMED FOR THIS CARD, WITH ZERO EMITTERS AND THE WRONG READING

`classifyClause` already carried `lifeTie`, and `effects.js` already evaluated
it as `act(n).hp === foe(n).hp`. It had **never once run**: the clause's
payload has no reader, so the whole line refused and nothing ever emitted the
condition. Its comment also asserted the collapse the card does not print —

> *"ties count as 'more' and 'less' both ways (Line Crossers)"*

— which is the two printed sentences folded into one. And a condition is the
wrong SHAPE besides: a `fx.conds` entry gates a payload, and this card has
none. Deleted.

**SO `condcensus` ASKS THE REVERSE DIRECTION NOW.** That file has asked *is
every condition the pool EMITS answered?* since v3.97, and a census that only
goes one way is half a census (v4.12, v4.17, v4.50). The reverse — *does every
condition the main loop ANSWERS have an emitter?* — found this one the day it
was asked. **30 answered, 3 orphans**, pinned: `arcDealt` (printed by Sigil of
Suffering, but as a DEFENCE condition), `atk` and `non` (printed by no pool
record at all).

**AND THE EMITTER WALK IS DEEP AND COVERS THE POWCARDS**, because a rider
carries conditions too (v3.95's `way:took`) and a hero or equipment ability is
parsed as a card that is not in the pool (v3.73). A scan that stops at
top-level `fx.conds` over pool records reports both as orphans — v4.00's false
POSITIVE twice over, and what the first draft produced. **The powCard legs are
measured to contribute nothing today and that is PINNED rather than assumed**
(v4.44): their sabotage is correctly silent, and the day one emits a condition
on its own the pin moves.

Measured: **exactly 1 unique card moves, `part` → `full`** (392 → 393 full,
13 → 12 part; 746 → 747 over the 797-record pool), floor re-pinned after
reading the diff; **the tie grant decides 3 of 339 life comparisons in 210
games**, and unlike v4.43's Hood and v4.49's Gun the piece **is** worn by
`defaultPicks`, so the route is genuinely driven; the ladder at three seeds is
byte-identical on **thirteen of fifteen heroes** and the two that move
(Lyath 5·5·4 → 5·6·4, Gravy 6·12·10 → 6·11·10) have intervals that overlap
entirely — **NOISE** by the standing rule (v4.40). **15 sabotages, 15 bite** —
and three of the first pass came back silent for reasons that were all mine: a
"counts as both" sabotage no fixture could express until the half-piece was
driven through the SIDE reader rather than only through the parse (v3.62), an
inverse anchor that appeared twice (v4.36), and a powCard leg another leg
covers (v4.50, verbatim).
...
a reading was **obeyed** rather than made — and it covered seven of
fifteen heroes. Writing the other eight is the whole of this version, and
it turned up two things no other tool could see. **34 scenes → 49.**

### EPHEMERAL WAS READ THREE WAYS ACROSS TWO FILES
...
any keyword — this project states that in four separate places — so both
of judge's readers matched **nothing, ever**.

So **Crouching Tiger reached the graveyard at the table** while the
trainer correctly dropped it. A card the rules *remove from the game*,
handed back to the player, on the board that is supposed to be the
CR-exact one. Fai's card, and a real card-advantage bug.

v3.01's shape again — a rule that exists on one board — with the twist
that the board it was missing from is the strict one, and that the reader
there was written against text that has never existed.

**`parser.isEphemeral` is the one reader**, and it asks `printedKw` rather
than `hasKw` (v2.84's three questions): a card that merely *mentions*
ephemeral in a sentence does not have it, and removing it from the game
would be the golden rule broken at the keyword level. No pool card tells
the two predicates apart, so the drill uses a synthetic near-miss —
v3.73's Crash-and-Bash discriminator, one keyword over.

### AND THE DEAD `rune` SIDE FIELD IS GONE

CLAUDE.md has said since v2.23:

> *"There is no `sd.rune` field any more — a drill would fail if one came
> back."*

**Both halves were false.** The field was still declared in `makeSide`,
still shipped down the wire, and **read by nothing at all for sixty
versions** — while `sides.js`'s own comment two lines above cited `rune`
as an example of a field *already retired*, reasoning from the thing it
...
`WIRE_V` bumps to 2 because the payload shape changed. The symmetry ledger
moves **47 → 46**, deliberately (v3.29). And the drill that claim promised
now exists — it holds every derived count (`rune`, `frost`, `rot`, `fra`)
to having no stored twin.

### And a stale coverage claim

CLAUDE.md's v3.20 note says *"Mounting Anger went none/part → **full**"*.
The audit says **`part`**, and v3.53's own measurement explains why: its
trigger is `hits`, and **zero pool cards** set `fx.optCost` on that
trigger. Corrected.

### The fixtures, again

Three of the eight scenes were wrong before the engine was, each in a way
this project has already named:
...
- the Viserai rite scene took *"the first Runeblade non-attack in his
  ...
  on **Mauvrion Skies**, whose own text queues a Runechant. A fixture that
  depends on a shuffle has not named what it is testing. It names the card
  now.
...
- the Fai scenes asserted two things I had not measured.
