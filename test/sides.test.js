...
test("symmetry gap: coverage — how much of a hero each seat carries", () => {
  ...
  assert.equal(gap.fields, 46);   /* +buffQ v2.30, -frost v2.74, -rot -fra v3.09, +nextTurn v3.29, +gaNextQ v3.31, +costOff v3.32, +instantNextQ v3.37, +heroTapped v3.48, +defCapNext v3.64, +wardRider v3.67 */
...
test("symmetry gap: migration — what has moved onto sides[]", () => {
  const gap = S.symmetryGap();
  assert.equal(gap.nativeForPlayer.length, 46);   /* … +costOff v3.32, +instantNextQ v3.37, +heroTapped v3.48, +defCapNext v3.64, +wardRider v3.67 */
