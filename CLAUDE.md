...
npm test
```
This is `node --test "test/*.test.js"` — **2931 drills** at v4.58.
...
> of times**. Your attacks **this combat chain** get +1{p} for each Light card
> charged this way."* — V OF THE VANGUARD, the pool's ninth FOR-EACH record

`tools/approx.js` carried this as `charged-this-way-count` and named **two**
missing halves. **The half the record was most confident about was wrong**, and
the cheapest diagnostic in this project found it in one call (v3.79, v4.43).

The record said the subject *"lands nowhere near `perBoost`'s site"* so
v4.48's anchor *"is right to refuse it"* — a missing MECHANISM for the standing
grant. **Measured, `atkBuff`'s reader has existed since v3.87 and its own
comment block names THIS CARD.** What refused was the printed WORD ORDER:
...
Night's Embrace prints the window at the END and this prints it right after
the SUBJECT — **v3.36's rule verbatim** (the database prints both spellings at
once) and **v4.48's dead `perBoost` one reader over**. A recorded reason is
only as good as the day it was measured (v3.69); the way to check one is to
ASK THE ENGINE (v4.09).

**THE PLURAL IS WHAT KEEPS `buffQ`'s FAMILY OUT, AND THE BLAST RADIUS SAID
SO.** The anchor read `attacks?`, harmless **only while the window had to come
LAST** — *"your NEXT arrow attack THIS TURN gets +3{p}"* prints its window
mid-clause too, so accepting either position made the optional `s` live and
this reader swallowed the whole single-shot family: **31 records across ten
cards**, every one turned into a grant that is never spent. v3.87's own
distinction inverted, in the direction that steals games, and v3.72's rule one
reader over. The plural is a RULE rather than a coincidence of this pool —
*"your next attack"* names ONE card by construction.

**THE COUNT HALF WAS REAL AND IS BUILT.** `fx.chargeCost.multi` has read the
printed *"any number of times"* since charge was built with **nothing
consuming it** (v3.55), so the pool's one `multi` record was offered a SINGLE
charge — weaker than printed, `tier: full` on a clause that WAS read.
**The OFFER shrinks, never the hand**: a charge is settled in `execute` three
hundred lines after the last answer, so nothing has left the hand while the
offer is re-made and one uid charged twice moves one card and counts two.
`chargeOffer` takes what is already picked as a fourth OPT-IN parameter
(v3.58) and stays the ONE reader; a DECLINE ends the charging, which is
upstream's own wording and the only reading under which the sheet terminates.
**And `_chargeUid` became a LIST rather than gaining a sibling** (v3.61) — the
single charge is a list of one, so there is no second record to disagree.
...
PAID, off `_chgWay` (the `_discWay` family, v3.62), with the class read off
the **structured array** (v2.39, v2.44): `tt` carries stray words on five
records and *"Lightning"* is not *"Light"* (v4.25's fallback trap).

**IT IS ALSO THE FIRST COUNTABLE THAT RIDES AS A PARAMETER RATHER THAN AS ITS
OWN OP KIND**, because the line grants to a STANDING set of attacks rather
than pumping the resolving card. So `test/foreach.test.js`'s emitter census
learned a **second landing site** and pins which one each countable takes,
both directions (v4.17) — asked for an op kind alone it reported the new
countable as having no emitter, which reads exactly like v4.48's dead
`perEquipDef` and would send the next reader hunting a defect that is not
...
a `+0{p}` entry here is that bug wearing an entry. In a training sim the feed
is the lesson (v3.60): a player who declined the charge is told that is why.

**AND THE BUILD'S OWN FIRST DRAFT HAD v4.09's DEFECT.** `_chgWay` is a
per-resolution trace and **an ATTACK's ops ride to RESOLUTION**. Driven with a
defence reaction in the reaction window, the next `execute` reassigned the
trace and the grant announced *"grants nothing"* — the count went **2 → 0**.
That is the hazard `pend.fused` already rides on `pend` for: a
DECLARATION-TIME fact the resolution needs. **`chargedRec` is the ONE reader**
of whichever lifetime is live, and it carries `{pitch, ty}` rather than card
objects, because `pend` is a GAME_KEY that ships whole and uninterned (v2.49).
**`pend.chargedPitch` → `pend.chargedWay`, the name moving with the shape**;
`WIRE_V` **10 → 11**, because a v10 client handed a v11 charge pending drops
`picked` and `multi`, re-offers nothing and charges only what the last answer
named (v3.98, v4.12, v4.26).

**THE LADDER IS BYTE-IDENTICAL AT THREE SEEDS ON BOTH SIDES, AND THE REASON IS
THE POLICY.** Measured over Boltyn's 28 ladder games: the card reaches the
table **141** times, the charge is **OFFERED 125** times and **TAKEN 0**,
because `sparring.act` declines a price it cannot weigh (v4.24, v4.33). So the
grant resolves at a count of zero and says so **14** times — the new op's ZERO
branch is genuinely driven while its accept path reads 0 on the ladder, which
is a number about the POLICY rather than the route (v4.29, v4.41, and v4.48's
own Boltyn line). The accept path is driven end to end by
`test/vanguard.test.js` and by a scene instead, so it is not left with no
caller (v3.50).

**28 SABOTAGES, 28 BITE — AND THE ONE THAT CAME BACK SILENT WAS A DRILL GAP.**
Both colour gates were rewritten to ask whether any MEMBER of the record
matches, and they are answered in DIFFERENT PLACES: the declaration-time one
in `execute`'s condition loop, the hit-time one inside `linkPayload`'s own
smaller evaluator (v3.96). Neutering the first failed a drill; neutering the
second failed nothing, because the only drill naming Light the Way asserted on
the PARSE. **A gate nothing drives is a gate a later change can delete in
silence.** It is driven now, with the ACTION POINT as the observable.

**AND THE HARNESS HAD TWO FAULTS OF ITS OWN.** Its scenes reader spelled the
summary line's separator as an ERE `.` and that separator is a MIDDOT — two
bytes in UTF-8 — so the line was missed and reported as `-1`, which the bite
test read as SILENT: a scan aimed at the wrong shape (v3.81, v4.07) inside the
instrument built to catch them. And its residue check grepped for a MARKER,
which a deletion-shaped replacement does not carry (v4.37); it asks **git**
now. **Writing a failing drill into `test/` while a pass is running inflates
every later count** — v4.36's rule with the causality reversed, and the five
readings at risk were re-derived rather than trusted.

**AND TWO SENTENCES INSIDE `chargeOffer` WERE STALE** — *"none prints
`multi`"* and *"the drill pins that SET empty"*. **Both stopped being true at
v4.48**, which widened the spelling and brought this card in, and that drill
has pinned the set as exactly this card ever since. A measurement rotting
inside the function the next reader would open.
...
one read a **FLAT +1** — the loose `this gets +N{p}` matcher and the loose
`defBuff` one both drop everything after the pip:
...
| Salt the Wound | `+1{p} for each attack that has hit this combat chain` | a flat `self: 1` |
| Big Blue Sky | `+1{d} for each blue card you've pitched this turn` | a `defBuff` op that only LOGS |

**WRONG IN BOTH DIRECTIONS AT ONCE, WHICH IS WHY NO TOOL HERE COULD SEE IT.**
At a count of 0 the engine grants a point the card does not; at 2 or more it
grants less than printed — the direction the one-sided sweep is built not to
look in — and all eight records read `tier: full`, so coverage is blind too.
**The two readings AGREE at a count of exactly 1**, so a fixture at one count
sees nothing and the pair either side is what bites (v3.92, v3.99).

**AND THE TWO OPS ALREADY BUILT FOR THE SHAPE HAD ZERO POOL EMITTERS.**
`perEquipDef` and `perBoost` were anchored on *"gains +X{p}, **WHERE X IS THE
NUMBER OF** …"* and **no record prints that** — measured over 797, the printed
form is *"+N{p} **FOR EACH** …"* on every one. Two carefully-reasoned fire sites
that had never once run: dead rules code that reads like a rule (v4.11). And
v4.20's argument about `piercing` (*"Fender Bender's is +N for EACH equipment"*)
was a comparison drawn against a reader with no card.

**THE DRILL THAT PINNED `perBoost` WROTE THE STALE WORDING OUT BY HAND** —
v3.00's editorial drift in its sharpest form yet: the fixture WAS the old text,
so it stayed green against a wording the database had stopped printing. It is
driven off the real record now, and it **moved** to `test/foreach.test.js`,
because `parser.test.js` takes no database by design — which is exactly why the
claim could only ever be made by typing the text out.

**AN UNKNOWN COUNTABLE REFUSES, and that is the load-bearing half rather than
caution: falling through IS the bug.** A countable the engine cannot count,
read as a flat +N, is the defect itself (v2.29).

**IT IS KEYED BY THE PRINTED SYMBOL, AND `{d}` IS A SEPARATE TABLE.** An op
lands on the resolving attack's POWER; a `{d}` countable moves what a card is
worth at the WALL — `defendValue`'s number, on a card that may never be on the
chain (Big Blue Sky is a Defense Reaction). One flat table would fire a `{d}`
countable as a power pump the day somebody added one. `PER_COUNT["{d}"]` is
pinned EMPTY with that reason (v4.33's `multi` rule); `fx.defSelf.per` carries
it and `effects.defPerCount` counts it.
...
THEM.** A Draconic LINK exists whether or not it connected; this asks about a
**HIT**, so CR 7.5.5 makes `dmg > 0` the test — `linkPayload` pushes an attack's
link UNCONDITIONALLY at `dmg: total`, so a swing blocked to nothing is on the
strip at zero. The strip carries ARCANE entries too (v4.39), which is why the
kind is tested. The asking attack is not yet on it (v3.99), which is what *"have
hit"* says.

**BIG BLUE SKY'S CLAUSE WAS A RECORDED REFUSAL BEING CLAIMED ANYWAY.**
`parser.js` has listed it as deliberately unread since v3.24 while the loose
`defBuff` matcher took it into an op `runOps` only LOGS — so `tier: full`, the
feed said *"+1 defense to the wall"*, and **no number moved on either board**.
It needed no machinery: `defendValue` has been the one reader of what a defender
is worth since v3.23 and `defSide` is already the CONTROLLER, so the pitch zone
is right there (v3.58, v3.73). Blue is **pitch 3**, read off the printed value,
because the colour is not a field.

**AND THE CENSUS v3.99 RAN BY HAND IS STANDING NOW** — the third this project
had run once (v4.42 promoted two others), and in the direction that hand-run
missed: **an op with a fire site and NO emitter**. Both halves pinned (v4.17).

Measured: **8 records' ops move, 7 more gain a `chargeCost`, 0 tiers move**
(392 / 13 / 0 unchanged — v4.18's point, and `pool-deck-complete` intact
because the multiplier was BUILT rather than the clause refused); **my own scan
found a NINTH for-each record** and corrected a pin written from memory (v4.09);
the ladder at three seeds is **byte-identical on eleven of fifteen heroes**,
four inside the band, Dash consistently +1 but with overlapping intervals, so
NOISE by the standing rule (v4.40); **26 sabotages, 26 bite.**

### THE SAME DEFECT ONE READER OVER — THE CHARGE COST (v4.48)

**Sixteen pool records print an additional-cost charge. NINE say *"your HERO'S
soul"* and SEVEN say *"your soul"*, and `fx.chargeCost`'s anchor required the
word "hero" — so the seven read NOTHING.** v3.36's rule verbatim (the database
prints both spellings at once), and **v4.41 had already taught `onChargeSoul`
both and left this reader on one**: v3.53's *a fix for one matcher is not a fix
for the shape*, v4.21's *fix the family*.

**SIX OF THE SEVEN ARE LIVE, `tier: full`, WITH A RIDER THAT COULD NEVER FIRE.**
Beaming Bravado ×3 prints *"if a **YELLOW** card is charged this way, this gets
+1{p}"* and Light the Way ×3 *"when this hits, if a yellow card was charged this
way, this gets **GO AGAIN**"* — both parse, both gated on a charge that never
happened, so the +1 and an ACTION POINT (CR 5.3.5) were unreachable. All six are
**Boltyn's**. Driven: declined → 3 power, a YELLOW → **4**, a RED → 3 with *"the
card charged this way wasn't the right colour"* — and the RED row is the one
that bites, because a colour-blind reader passes the first two perfectly.

**`multi` WAS PINNED EMPTY AT v4.33 AND THIS IS THE DAY IT MOVED.** The widening
brought V of the Vanguard in, the pool's only `multi` record. **The decision is
to offer ONE charge and say so** — weaker than printed, visible (its payload
clause still reads `skip`), recorded as `charged-this-way-count` with a probe
that goes red the day the count is built. Charging more than once silently is
the other direction.

**AND THE OFFER'S FEED LINE NAMED ONE SEAT AND AGREED WITH THE OTHER** — seat 0
is literally named *"You"* (v2.83), so it read *"You may put a card from hand
into **their** hero's soul"* on all 16 records. `game.sp` (v4.22) existed for
exactly this and inflects the NAME rather than replacing it with "your".

**BOLTYN IS BYTE-IDENTICAL ON THE LADDER, AND THAT IS THE POLICY.**
`sparring.act` DECLINES the charge (v4.33's recorded decision), so the six
newly-reachable records are offered and refused — a number about the POLICY
rather than the route (v4.24, v4.29, v4.41).

### THE LAST ONE-BOARD ROUTE, AND A `{t}` NOBODY CHARGED (v4.47)
...
**v3.01's shape, and the LAST member of that family**: v3.04's seventeen
dead equipment abilities, v3.39's hero branch, v3.44's ally attack,
v3.84's aura attack, and this.
...
decked, three tokens with no creator yet. **Both halves are pinned** — the
seven that read and the four that refuse — because pinning the readers
alone cannot see a name leaving the list (v4.29).

**`build.boardPow` IS THE SHARED READER, AND IT IS THE THIRD POWCARD
BUILDER.** The other two comment blocks have named it *by name* since
v3.79 — *"there are three (hero, here, and `boardPow`)"* — while it sat
inside `Battle` as a React closure. So v3.63's rule (add a flag to one
builder, grep the others) was a grep across two files, **one of which no
drill could reach, and it had already cost the cost flags**: `equipPiece`
stamps five and this stamped NONE.
...
about GEAR — so those three are stamped as guards. `_selfBanish` carries
`_banishGear` and `_flipUp` carries `_flipGear`, and **both `execute` and
`abCostWhy` resolve those uids against `sd.gear`**, so an arena permanent
stamped with either has its ability refused forever or banishes nothing at
all. **Half-building a cost is worse than the honest gap** (v3.23), so
they are left off and a drill fails the day a pool record prints one.

**THE LEGALITIES ARE THE GEAR BRANCH'S, OUT OF THE SAME BODIES** —
`abCostWhy` (v3.99), `activateIfOk`, `abWindow`, `rxTargetWhy` and
`effCost` with the game's half (v3.80). **`abCostWhy`'s call count is a PIN
and went 2 → 3**, so a fourth activation route is a deliberate edit in both
places.

**AND BUILDING THE ROUTE EXPOSED A COST NOBODY CHARGED, ON EITHER BOARD.**

> *"**Instant - {t}:** Target attack gets +1{p}."* — CONCEALED OBJECT,
> Lyath's, ×2

Driven three times on one turn it queued **+1, +2, +3** for free —
unbounded, STRONGER than printed, and `tier: full` throughout because the
clause IS read, so coverage is blind and the one-sided sweep looks the
other way. **v4.46's own defect one zone over, found the same way**: make
the cost legible, then drive it.

**MEASURED: exactly ONE readable arena record prints a `{t}`, and it is the
only one of the seven that does NOT print "destroy this"** — which is why
nothing else limited it; the other six are spent by their own cost.
...
own untap step lifts (CR 4.4.3d) and judge's step (d) already clears it;
`weaponUsed` comes back at every turn boundary for both seats. **The drill
drives a whole turn cycle from BOTH seats**, because that is the only thing
that can tell the two records apart. And it **SAYS SO** — the line names
the permanent, names the seat through `sv`, and names **CR 4.4.3d** rather
than a turn number, because *"until your untap step"* is what a player
plans around.

**`judge.boardAbilityOf` IS `boardAttackOf`'s SIBLING**, so the arena tile
offers it while reading no card text (v3.84's contract) — and without it
the tile v4.45 had just given a handler still had no caller for any of the
seven, which is **v3.50's sentence for the sixth time**. An ability carries
no attack-target, so it sends the bare action; v4.45's own census learned
that exemption as an **allow-list with both halves asserted**.

**THE POLICY DECLINES, AND THAT IS A STATED CHOICE WITH A REASON.**
`sparring.act` never activates one, so the route reads ZERO on the ladder
— v4.24's standing rule, because six of the seven pay with the PERMANENT
and whether a Gold is worth a card is a judgement about card text
`sparring.js` reads none of. v4.38 measured the cost of letting this policy
make a timing judgement it cannot: three heroes went 22 → 4, 15 → 2, 16 →
3. So the zero is about the POLICY (v4.41, v4.33, v4.29) and the route is
DRIVEN instead. **The `tap` counter still learns the third spelling**,
because a counter that cannot see its own event is the defect v4.46
rewrote that block to remove.

**THREE DRILLS WERE GREPPING A RULE THAT MOVED** (v3.22, v3.28, v3.94 —
fourth outing): two grepped `index.html` for `boardPow` and now DRIVE
`build.boardPow`, which is driving the trainer's own reader because the
trainer's `boardPow` **is** it. And **`parseHeroPower` LEFT `keycensus`'s
trainer-only set** — its one caller there was `boardPow`, so the bridge
alias became a global nothing calls (v3.77, v4.11, v4.33) and is deleted.
**A name leaving a census is as deliberate an edit as one arriving**
(v4.12).

**THE POOL IS THE WRONG ORACLE FOR WHAT A BUILDER BUILDS.** The extraction
was measured against the old body over the whole pool and reported **TEN
differences** — every one a hero or an Equipment record, which can never be
a board entry. Restricted to what `boardPow` can be handed
(`destination === "arena"`, non-ally): **7 records, 0 differing.** v4.32's
lesson one builder over, and the first draft asked the wrong set exactly as
that version's did.

Measured: the ledger record is **CLOSED** with its probe turned round
(31 → 32 records, 12 → 13 stated, 7 → 6 open, 12 → 13 closed); **no pool
record's parse or tier moves** (392 / 13 / 0); **the ladder is
BYTE-IDENTICAL at three seeds on both sides**, run rather than reasoned
about (v4.43). **29 sabotages, 29 bite — and FIVE were my own fixtures
first**: two window guards sharing a state where one was unreachable behind
the other (v3.62, v4.34), an ally fixture whose payload `classifyClause`
refuses anyway so the `kind` guard was never the thing refusing, a source
scan that **`if(false && …)` walks straight through** (v4.00, verbatim — so
the scan refuses that shape by name and its reach is STATED), and a revert
anchor `equipPiece` prints identically (v4.36).

### `npm run eight` — READ A GAME, DON'T COUNT IT (v4.46)

```
npm run eight                    # eight entrants, Bo1, 7 ties, ~2 min
node tools/cup8.js --name "X"    # a different draw, reproducibly
node tools/cup8.js --full        # every timeline row beside the call
```
...
be **READ** — and reading seven games found **four defects, three of them
invisible to every other tool here.**

**BEST OF ONE IS A TEST OF VARIANCE, AND THE REPORT PRINTS THAT.** This
project's own measurement: a 92% favourite loses a single game about once
in twelve, and the chair is worth about 2:1 in a close matchup. So nothing
this instrument produces is evidence about a deck, which is the claim a
bracket most invites and the one it cannot support.

**THE TWO VOICES ARE READERS, NOT AUTHORS** — `test/cup8.test.js`, and it
is a drilled contract because a commentator that narrates reads exactly
like one that reports (v4.27's `failstates.js` counting a keyword named in
a comment, wearing a headset). **Tally** gets ONE timeline row and may say
its feed lines and a life total reachable from the row before; **Ledger**
may add a reason the row carries — the swing, a route it fired, its CR
step. The drills are **synthetic** (v3.73), because every real row's
numbers are consistent with the game and an invented one would be
indistinguishable: a row at 20-20 whose lines carry no digit is what tells
a reader from an author.

**ONE DRIVE LOOP.** `selfplay.play` records a timeline **opt-in** (v3.58)
rather than the caller re-driving the game — a second loop in a tool is the
no-mirror rule broken where nothing watches it (v4.25) — and per-row route
kinds come off the event list's own LENGTH rather than by re-matching the
patterns, for the identical reason.

### A COUNTER THAT MEASURED THE OPPOSITE OF ITS FEATURE (v4.46)

**`tap` read `/tapped|taps/i` and fired 335 times in 15 games. Every single
one was `"(d) <name> untaps."`** — the end-phase step that announces itself
unconditionally every turn, in which **"un-TAPS" contains "taps"**. Not one
was a tap.

v3.81 records a counter that spelled the wrong word and reported ZERO. This
is that defect **with the sign flipped**, in the block where a number means
a FEATURE FIRED (v4.17): not a silence, a confident 335.

**`ally` WAS THE SAME SHAPE — 350 firings, 335 of them `"(a) Allies
recover."`**, plus three for a card NAMED **Rally** the Coast Guard, which
is v2.44's *"Reaction" contains "action"* and v4.25's *"Lightning Fusion"
falls back to "lightning"* — third outing, in an instrument. **95.7%
noise.**

**AND THE ROUTE ITS OWN HEADER NAMED HAD NEVER BEEN MEASURED.** An ally
ATTACK — v3.44's whole build — prints *"\<hero\> **sends** \<Ally\> — N
power on the chain"*, in which the word "ally" does not appear:
`from === "ally"` selects the VERB and nothing else. It fires **15 times in
15 games** and read zero for as long as the counter existed. **Two events
are two records** (v3.40), so `ally` is the deploy and `allyatk` the
attack.

> `ally` 350 → **12** · `allyatk` **0 → 15** · `tap` 335 → **10**

**THE END PHASE ANNOUNCES EVERY STEP BY NAME, INCLUDING THE ONES THAT DO
NOTHING** — deliberately (judge.js: *"a step that announces itself and does
nothing is worse than one that is missing"*). That is right for the FEED
and it means **the feed is full of the words a route counter wants**. Spell
the EVENT, never the word.

### A COST CHARGED IN TOTAL SILENCE (v4.46)

Aimed properly, `tap` had nothing to aim at: **zero** feed lines in 15
games mentioned a tap other than that announcement — while `heroTapped` was
true across **83 states of one Bravo game**. The only line the mechanic
ever produced was the REFUSAL (*"\<name\> is already tapped"*), so a player
learned their hero was tapped **by being refused a play**.

**IN A TRAINING SIM THE FEED IS THE LESSON** (v3.60, v4.24). The tap is not
cosmetic: `heroTapped` is a STATE only the controller's own untap step lifts
(CR 4.4.3d), so it is a cost still being paid on the opponent's turn — and
the board was announcing the UNTAP unconditionally while staying silent
about the half that costs something.

**AND THE ANNOUNCEMENT IS WHAT FOUND THE NEXT ONE, INSIDE A MINUTE.** That
is the whole argument for making a mechanic legible before trusting it.

### `from === "hero"` IS NOT "THE HERO ACTED" (v4.46)
...
`judge.js`'s own comment calls that route **"THE ABILITY ROUTE (v3.04)"**
and commits every non-weapon equipment ability down it — so the branch
fired for a Compass and then asked whether the **HERO's** printed line
contains `{t}`. **Two different cards, one route name, and the wrong one
answering.**

**MEASURED: 35 of 41 hero taps were an equipment ability tapping the hero
for a cost it never printed.** Three of fifteen heroes print one (Bravo,
Gravy Bones, Lyath — v3.48's count), and `heroTapped` blocks a later `{t}`,
so **Gravy Bones activating a Compass locked himself out of his own hero
ability for the turn.**

**WEAKER THAN PRINTED**, the direction the one-sided sweep is built not to
look in — and the clause was READ throughout, so coverage is blind too.
**`uid === "hpow"` is the discriminator and the powCard's own text cannot
be**: `build.js` strips the cost prefix off the ability, which is exactly
why the line reads the hero record instead.

Driven at the same pairing, seed and chair — the one tie where BOTH heroes
print `{t}`: **turn 37 → 23, 565 actions → 375, life -2 v 1 → -4 v 7**,
same winner. **41 hero taps → 7**, every one genuine. The ladder at three
seeds both sides is NOISE on all three heroes (v4.40), which is the honest
read for a defect that cost a once-per-turn ability only sometimes.

### A PROMPT'S RESULT CALLED THE READER'S OPPONENT "You" (v4.46)
...
**MEASURED: 55 feed lines over 15 games** opened with a hardcoded seat name
while both seats were named after heroes — half calling the reader's
opponent "You", half calling a named hero "The opponent". **It is v4.15's
own defect one variable over**: that version found 86 *"You soaks"* lines
and built `svName` to fix the VERB, which it did — against a name that was
wrong. **55 → 0.**

**THE FALLBACK IS THE TRAINER'S AND COSTS NOTHING THERE.** `index.html`
names seat 0 literally "You", so those lines are byte-identical; seat 1
stops being called "The opponent" when it has a name, which is v3.46's
direction. `test/promptvoice.test.js` drives **both** (v3.98) — a named
seat must be named AND a seat genuinely called "You" must still read in the
second person, or the fix is a regression wearing a fix's clothes.

**THE DEBT LEDGER COVERS `effects.js` ONLY.** `test/judge.test.js` pins 46
second-person literals there and `prompts.js` was never in it — which is
how six message sites shared one hardcoded subject for as long as they did.

### AND THE HARNESS HAD THE SHAPE IT WAS BUILT TO FIND (v4.46)

`cup8`'s report built its `flagged` set from `v.i` on the judges' rows —
**and those rows carried no `i`**, so the set could never be non-empty:
v4.11's *a guard that cannot express a bug*, in the instrument looking for
exactly that. Findings carry the row index now, **omitted entirely when no
timeline was asked for**, because a field that is always 0 reads as an
answer (v2.83's frozen `mode`). Verified by making the judge speak on
purpose and checking the finding landed on the right row.

**AND A NAME TAKEN FROM A NEIGHBOURING BLOCK THREW FROM INSIDE A REDUCER.**
The tap announcement's first draft used `srcName`, not in scope there —
v4.09 verbatim, caught because it was DRIVEN. One new drill also failed
against a correct engine by checking the wrong half of its own fixture: the
card prefix is `"Bravo — hero power"` and the seat is
...
action that spends the life is a pass rather than the attack. The booth says
so once — a reader who is not told reads it as a bug in the log. And
**overkill makes a life display ambiguous**: `[-4, 4].join("-")` renders
`"-4-4"`, which parses as 4 against 4 with a stray sign, so it prints
`a v b`.
...
> **BUILT AT v4.47, and the probe went RED the moment the branch landed** —
> which is exactly what an `open` record is for. `build.boardPow` is the
> shared reader and the record is `arena-ability-routed-at-the-table`,
> closed, with the probe turned round. **And building it exposed a cost
> nobody charged on EITHER board**: Concealed Object's printed `{t}`. See
> "THE LAST ONE-BOARD ROUTE, AND A `{t}` NOBODY CHARGED".
...
> — HOPE MERCHANT'S HOOD, SFA004, in Dash's and Fai's lists

**THE CHEAPEST DIAGNOSTIC IN THIS PROJECT, RUN BACKWARDS.** v3.79's rule is
*a card at `none` whose payload parses is a COST or a TRIGGER*; the sharper
form is to hand the SAME printed line a payload that HAS a reader:

| handed | `parseHeroPower` answers |
|---|---|
| `Instant - Destroy this: Draw a card.` | `{cost:0, sd:true, kind:"instant"}` |
| the Hood's real line | **`null`** |

Identical cost, prefix and window — so the destroy, the instant speed and
the powCard were **every one of them waiting on one sentence**, and
`build.js` built the piece no powCard at all. **v3.47's shape, SIXTH
outing: reading the payload is what creates the route.**

**AND `tools/approx.js` WAS WRONG ABOUT WHY IT WAS THERE.** The record named
the blocker as *"a ZONE MOVE (deck manipulation) — machinery `prompts.js`
does not have"*, and `moveCards` has routed a pick to the deck since the
module was written. **A recorded reason is only as good as the day it was
measured** (v3.69) — the way to check one is to ASK the engine, which is how
...
three ops would need `runOps` to thread *"how many did the last one move"*
between them, which is state no op carries (v3.71, v3.88). `shuffleDraw`
rides on the spec and `applyAnswer` reads `r.picked.length` — what that
field was added for (v3.41).

**THE ORDER IS THE CARD.** `applyPrompt` puts the chosen cards at the FRONT
of the deck, so with the shuffle dropped the controller draws back exactly
what they put down — every zone count correct, the feed saying the cards
went back, the hand unchanged. Drawing before shuffling is the same bug the
other way. **And what went back CAN come out again**: this is a
RE-RANDOMISATION rather than a cycle, which is why the deck is shuffled with
the returned cards IN it, and it is the line-finding the card exists for.
The draw cannot be short, and that falls out rather than being guarded — N
cards were just shuffled in.

**THE POOL'S FIRST MULTI-CARD PICK**, measured: every other `pickPrompt` is
`max: 1`. `maxAll` is the printed *"any number"* and `buildPrompt` resolves
it against the CANDIDATE POOL it has already filtered — never at the queue
site, which would re-derive the same filter, and never in the parse, where
`fxParse`'s `name|pitch` memo freezes a hand size (v3.39, v3.92).
*"Any number"* includes ZERO, so `min: 0` gives the sheet its **Choose
none**, and declining does not even touch the seeded stream (v2.04).
`shuffleDraw` is the **SEVENTH field to prove v2.34's rule**, now with
v4.42's census behind it.

**THE TAIL IS A CLOSED READING, AND THAT DECIDES WHERE THE RULE SITS.** The
shape is matched wide and an unreadable tail REFUSES, which takes the clause
away from the unanchored plain-draw matcher rather than leaving it read
half-way: measured, a *"…then draw TWO cards"* variant reads as a bare
`[["draw",2]]` — the payload with the entire printed cost gone (v3.00,
v3.60). So it sits ABOVE the plain draw rather than beside its `pickPrompt`
family, and **the premise is a drill**: widening that pattern to accept
*"that many"* fails a test instead of silently eating the shuffle.
*"Any number of"* is in the ANCHOR too, because `pickSubject` answers NULL
for the phrase — stripped first it would read *"shuffle A CARD … then draw
that many"* as unbounded, inventing a bound on the one card where the bound
is the decision.

**THE ROUTE COUNTER READS ZERO, AND THAT IS ABOUT THE LOADOUT.** Measured:
the Hood prints **0 defence** and Blade Beckoner Helm prints **1**, so
`defaultPicks` takes the Helm in the head slot of BOTH lists that deck it
and the piece is never worn in a driven game. Which armour to wear is a
deck-building judgement and `defaultPicks` reads no card text by contract,
exactly as `sparring.js` does not (v4.24). A player picks it on the loadout
screen, which is the route the drills seat explicitly — and that premise is
a drill as well, so a `defaultPicks` that starts ranking by text has to
re-measure the recorded reason.

Measured: **exactly 1 record moves, `none` → `full`** (390 → 391 full,
2 → 1 none); **the ladder is byte-identical**, verified by running it on
both sides rather than reasoning about it. **17 sabotages, 16 bite** — and
the two silent ones could not express their bug (v3.62): `applyPick` strips
a same-slot pre-seed, and making `moveCards` APPEND is unobservable behind
the shuffle, which is the property itself. **And the harness hit v4.37's own
lesson a third time** — a revert guard that asserted before writing left one
sabotage in flight, so the next script's "baseline" was taken under it and
read 24/0 like a silent sabotage. Revert first, then report, and re-derive
every count taken while a sabotage was live.
...
> **DISCHARGED AT v4.48, AND NOT BY ADDING LEGS.** That version corrected
> three attacks from a flat +1 to a printed multiplier, so the policy's own
> power arithmetic changed and **the same 32 deterministic legs play a
> different game** — reaching `spendCtr` and `avail`. A sample-dependent
> census moving when the ENGINE moves is the property rather than a fault,
> and the limit closed by the engine moving rather than the claim
> loosening.
...
> **AND THAT COMPARISON WAS DRAWN AGAINST A READER WITH NO CARD (v4.48).**
> `perEquipDef` was anchored on *"where X is the number of equipment
> defending it"* and Fender Bender prints *"for each equipment defending
> it"* — so it had **ZERO pool emitters** and its fire site had never once
> run. The distinction this entry makes is real; the twin it names was not
> reachable. Both read correctly now — see "FOR EACH" IS THE WORDING THE
> DATABASE PRINTS.
...
open: Quick Clicks' *"activate this only if you've played a Nimblism this turn"*.

**AND "HOPE MERCHANT'S HOOD — DECK MANIPULATION" WAS ON THAT LIST AND WAS
NOT TRUE (v4.43).** `moveCards` has routed a pick to the deck since
`prompts.js` was written; what was missing was one sentence in the parser.
A recorded reason is only as good as the day it was measured (v3.69) — see
...
6c. **PLAY IT** (`npm run play`) after any rules change — 210 self-play
   ...
   **`npm run eight`** is the third hat: eight entrants, Bo1, seven ties
   printed LINE BY LINE with two derived commentators and all four judges.
   It is evidence about nothing (a Bo1 is a variance test and the report
   says so) and it is the only instrument here that makes a game legible
   enough to read — which is what found v4.46's four defects, three of
   them invisible to every other tool in this list.
...
were decoration. `boardPow(b)` — **`build.boardPow` since v4.47**, because a
builder inside a React closure is why `judge.legal` had no arena-ability branch
at all — builds one lazily, keyed `"bp"+uid` so it cannot
